...
def get_lessons():
    ...
    lessons = retry_operation(fetch_all_lessons, max_retries=3, backoff_factor=2, user_id=token['user_id'])
    if lessons:
        return jsonify({"lessons": lessons}), 200
    return jsonify({"error": "Failed to retrieve lessons. Please try again later."}), 500

# Route to fetch a specific lesson by lesson_id
@lesson_bp.route('/<lesson_id>', methods=['GET'])
@limiter.limit("10 per minute")  # Limit for specific lesson fetch requests
def get_lesson(lesson_id):
    ...
    # Retry fetching the specific lesson in case of database failures
    lesson = retry_operation(fetch_lesson_by_id, max_retries=3, backoff_factor=2, user_id=token['user_id'], lesson_id=lesson_id)
    if lesson:
