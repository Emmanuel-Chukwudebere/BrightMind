...
# Rate limiter configuration to control excessive requests
limiter = Limiter(
    key_func=get_remote_address,  # Identifies users by IP for rate limiting
    app=ai_bp,  # Apply rate limiting to the ai_bp blueprint
    default_limits=["100 per minute"],  # Global rate limit for AI-related routes
    strategy="moving-window"  # Prevents bursts by applying limits over a rolling window
)
...
def ask_ai_route():
    ...
    # Validate the JWT token to ensure authorized access
    if not validate_jwt_token(token):
        return jsonify({"error": "Unauthorized access"}), 401

    log_request('/ask-ai')
    
    # Retry AI operation in case of temporary service failure
    ai_response = retry_operation(ask_ai, max_retries=3, backoff_factor=2, question=question)
    if ai_response:
