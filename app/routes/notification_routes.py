...
def send_notification_route():
    """
    API endpoint to send an FCM notification based on a specific event.

    Request JSON:
        user_id (str): The target user's ID.
        message_title (str): The notification title.
        message_body (str): The notification body.

    Returns:
        JSON response with the result of the notification sending.
    """
    token = request.headers.get('Authorization')
    user_data = validate_jwt_token(token)
    ...
    # Fetch the target user's FCM token and notification preferences
    user_ref = db.collection('users').document(user_id)
    user_data = user_ref.get().to_dict()
